...
(() => {
  ...
  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)');
  const enhancedScroll = window.matchMedia('(min-width: 1180px) and (min-height: 640px) and (any-hover: hover) and (any-pointer: fine)');
  const shortViewport = window.matchMedia('(min-width: 1180px) and (min-height: 640px) and (max-height: 850px)');
  ...
  const needsHeaderRoom = element => shortViewport.matches && element.matches('.section.viewport-module, .split-section.viewport-module, .split-layout.viewport-module');
  const destinationTop = element => {
    const top = targetTop(element);
    if (element === footer || !needsHeaderRoom(element)) return top;
    const headerHeight = fixedHeader ? Math.round(fixedHeader.getBoundingClientRect().height) : 72;
  ...
  const currentIndex = () => {
    ...
    targets.forEach((target, index) => {
      const distance = Math.abs(destinationTop(target) - position);
  ...
  const scrollToTarget = index => {
    ...
    const destination = destinationTop(targets[bounded]);
  ...
  const updateScrollMode = () => {
    enhancedScrollEnabled = enhancedScroll.matches;
  ...
  window.addEventListener('wheel', event => {
    ...
    if (wheelBurstActive || animationActive || performance.now() < wheelLockUntil) return;
    ...
    wheelLockUntil = performance.now() + (reduceMotion.matches ? 320 : 1050);
  ...
  const fitModules = () => {
    ...
    modules.forEach(module => {
      ...
      requestAnimationFrame(() => {
        ...
        const currentZoom = Number.parseFloat(content.style.zoom) || 1;
        const contentHeight = Math.max(content.scrollHeight, contentRect.height / currentZoom, 1);
        const contentWidth = Math.max(content.scrollWidth, contentRect.width / currentZoom, 1);
        const compactModule = needsHeaderRoom(module);
        const moduleStyle = compactModule ? window.getComputedStyle(module) : null;
        const verticalPadding = moduleStyle ? parseFloat(moduleStyle.paddingTop) + parseFloat(moduleStyle.paddingBottom) : 0;
        const verticalRoom = compactModule
          ? Math.max(1, moduleRect.height - verticalPadding - 16)
          : Math.min(moduleRect.height - 16, content.clientHeight || moduleRect.height - 16);
        ...
        const targetZoom = scale < .985 ? Math.max(compactModule ? .64 : .72, scale) : 1;
        if (Math.abs(currentZoom - targetZoom) > .002) {
          content.style.zoom = targetZoom < .999 ? String(targetZoom) : '';
        }
      });
    });
  };
